...
function p(string $msg = '', string $color = ''): void {
    $p = ($color === CLR_GREEN) ? '  OK ' : (($color === CLR_RED) ? '  FAIL ' : (($color === CLR_YELLOW) ? '  WARN ' : '     '));
    echo $color . $p . $msg . CLR_RESET . "\n";
}
...
function record(string $name, bool $pass, bool $isWarning = false, string $detail = ''): void {
    ...
    if ($pass && !$isWarning) {
        ...
        $GLOBALS['warnings']++;
        p($name . ($detail ? " \u{2014} {$detail}" : ''), CLR_YELLOW);
    } else {
        $GLOBALS['failed']++;
        $GLOBALS['fail_details'][] = "FAIL {$name}" . ($detail ? " \u{2014} {$detail}" : '');
        p($name . ($detail ? " \u{2014} {$detail}" : ''), CLR_RED);
    }
}
